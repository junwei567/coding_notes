def leap_year(year):
    """
    Determines if a year has 29 days on Feb
    """
    if year % 4 != 0:
        return False
    elif year % 100 != 0:
...
def day_of_week_jan1(year):
    """
    Using this formula, my calendar will know what is the first day of the year
    """
...
def construct_cal_month(month_num, first_day_of_month, num_days_in_month):
    """
    Firstly, use first day of month to find out which day the month starts from, 
    fill the month with the number of days
    """
...
def print_cal_month (list_of_str) :
    """
    Use a list of string from construct_cal_month(), and converts string into print
    """
    ret_val = ''
...
# modified function to only give a particular month
